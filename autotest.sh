...
# /home/qspace/workspace/josephyou/bin/micromamba run -n sglang2 python3 -m sglang.launch_server --model-path Qwen/Qwen1.5-14B \
...
# /home/qspace/workspace/josephyou/bin/micromamba run -n sglang2 python3 -m sglang.bench_serving --backend sglang \
...
# for rate in $(seq 3.5 0.25 6); do
    # echo "Running with request-rate: $rate" | tee -a "$LOG_FILE"  # 输出当前的 request-rate 值并追加到日志文件
/home/qspace/workspace/josephyou/bin/micromamba run -n sglang2 python3 -m sglang.iter_search_best_qps --backend sglang \
        --host 127.0.0.1 --port 8080 --dataset-name random \
        --tokenizer Qwen/Qwen1.5-14B --model Qwen/Qwen1.5-14B \
        --random-output-len 1024 --random-input-len 4096 \
        --random-range-ratio 0.5 --seed 1234 \
